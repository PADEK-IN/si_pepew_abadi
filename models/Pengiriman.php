...
class Pengiriman extends BaseModel {
    ...
    public function create($id_pesanan, $tanggal, $alamat, $status = 'diperjalanan') {
        $stmt = $this->pdo->prepare("INSERT INTO pengiriman (id_pesanan, tanggal, alamat, status) VALUES (?, ?, ?, ?, ?)");
        return $stmt->execute([$id_pesanan, $tanggal, $alamat, $status]);
    }

    public function update($id, $id_pesanan, $tanggal, $alamat, $status = 'diproses') {
        $stmt = $this->pdo->prepare("UPDATE pengiriman SET id_pesanan = ?, tanggal = ?, alamat = ?, status = ? WHERE id = ?");
        return $stmt->execute([$id_pesanan, $tanggal, $alamat, $status, $id]);
    }
    ...
    public function getPengirimanByDateRange($startDate, $endDate) {
        $query = "SELECT * FROM pengiriman WHERE tanggal BETWEEN :startDate AND :endDate";
